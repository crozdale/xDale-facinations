...
function VaultCard({ vault }) {
  return (
    <Link
      to={`/vault/${vault.id}`}
      className="relative block rounded-2xl border border-black/10 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md overflow-hidden"
    >
      {/* Micro XER watermark (cannot bleed) */}
      <img
